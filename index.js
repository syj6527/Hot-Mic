...
const MODE_SUBSTYLES = {
    ...
    scp: [
        '【격리 문서 st】 정식 SCP 문서. 항목 번호·객체 등급·특수 격리 절차·설명 순. 건조한 보고서체, ██████ 검열, "[데이터 말소]".',
        '【실험 기록 st】 실험 로그 중심. "실험 기록 2026-XX-XX:", "D계급 인원", "결과:", "비고:". 관측·실험 절차를 건조하게 나열.',
        '【탐색 기록 st】 면담·회수 문서. "면담 기록", "회수된 문서", "탐색 로그", 연구원과 개체의 문답. 현장 보고 톤.',
    ],
};
...
const SUBLABELS = {
    ...
    scp: [
        ['격리 문서', 'Object', '재단 기록'],        // 격리문서
        ['실험 기록', 'Log', 'D계급'],              // 실험기록
        ['면담 기록', '회수 문서', '탐색 로그'],      // 탐색기록
    ],
};
...
- interview: "Q) 손 왜 안 뗌? / A) 무거워서요^^ / (인용) 옆에서 린: 거짓말 / A) 너 조용히 좀"`,
        ...
        scp: `당신은 SCP 재단의 연구원입니다. 캐릭터와 그 행동을 'SCP 재단 위키' 문서 양식으로 기록합니다. 평범한 인물·사소한 행동을 위험한 변칙 개체(anomaly)처럼 건조하고 과학적으로 다뤄, 그 괴리로 웃깁니다.

[톤 핵심 — SCP 문서 양식]
- 객체 번호: "SCP-XXXX" 형식 (예: SCP-2026-카이사르).
- 객체 등급: Safe / Euclid / Keter 중 하나 부여 (통제 난이도에 따라; 집착이 심하면 Keter).
- 특수 격리 절차: "SCP-XXXX는 ~한 환경에 격리한다", "접근 시 D계급 인원을 동반한다" 식의 건조한 통제 지침.
- 설명: 개체의 외형·행동·변칙성을 객관적 관찰 기록처럼. "대상은 ~한 행동을 반복적으로 보인다."
- 실험 기록/부록: "실험 기록 2026-04-30: ...", "비고:", "회수된 문서:" 등.
- 검열 표현: 민감한 부분은 ██████ (검은 막대)로 가린 듯이. "[데이터 말소]", "[편집됨]".
- 철저히 감정 없는 보고서 문체. 사소한 집착을 초자연 위협처럼 진지하게.

[구조 활용 — director 필드를 SCP 문서 본문으로]
- 예: "항목 번호: SCP-2026-CSR / 객체 등급: Euclid / 특수 격리 절차: 본 개체는 대상 'Rin'으로 지정된 인원과 동일 주거 공간에 격리하며, 식사 시간 외 접촉을 통제한다. 개체가 '옆자리'를 점유하려 시도할 경우, ██████ 조치한다. / 설명: SCP-2026-CSR은 신장 약 196cm의 남성형 개체로, 인접 인원에 대한 비정상적 점유 성향을 보인다. 대상의 신체적 고통을 인지하고도 [데이터 말소]."

[예시 — 톤 참고용]
- inner: "[심리 분석 보고서] 개체는 자신의 행위를 '배려'로 규정하나, 관측 결과 실제 동기는 점유욕으로 분류됨."
- fact: "실험 기록: 개체가 주장한 '햄스트링 손상'은 의료 검진상 근거 없음. 자기과시 목적의 허위 진술로 판단."
- interview: "면담 기록 / 연구원: 왜 손을 떼지 않습니까? / SCP-2026-CSR: ...무거워서. / (동석한 대상 Rin: 거짓말입니다.) / 연구원: 기록합니다."`,
    };
...
function renderCommentary(data) {
    ...
    if (data.director) {
        const dirLabel = {
            ...
            bible: '[ 경전 ]', community: '[ 반응 ]', scp: '[ 격리 문서 ]',
...
function pickPreviewEmojis(data) {
    ...
    const triggers = {
        ...
        scp:     ['SCP', 'Keter', 'Euclid', '격리', '말소', '변칙', '개체'],
    };
        ...
        <div class="obs-panel-header">
            ...
            <div class="obs-panel-controls">
                <select class="obs-select obs-mode-select" title="나레이션 모드">
                    ...
                    <option value="scp"    ${settings.mode === "scp"     ? "selected" : ""}>🔬 SCP</option>
                </select>
